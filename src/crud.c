...
unsigned char is_a_db_file(int fd_data, char* file_name,Header_d *hd_caller)
{
...
unsigned char get_rec(int fd_dat,int fd_inx, int* index, char* key, Record_f*** recs, char* file_name, int lock)
{
	if(lock == LK_REQ)
	{
	...
	}else
	{
		do
		{
			...
			Record_f** rec_n = realloc(*recs,(++rec_arr_s) * sizeof(Record_f*));
			if(!rec_n)
			{
...
unsigned char schema_control(int fd_data, unsigned char* check_s, char* file_name,char* data_to_add,
				 Record_f** rec_c, Schema** sch_c, unsigned char update,
				 int *pcount, int crud)
{
	...
	*rec_c = rec;
	...
	if(update)
	{
		...
		*check_s = check;
		...
		(*sch_c)->types = calloc((*sch_c)->fields_num, sizeof(ValueType));
		if(!(*sch_c)->types)
		{
			printf("calloc failed. %s:%d.\n", F,L-3);
			free(buffer), free(buf_t), free(buf_v);
...
unsigned char write_rec(int fd_data, int fd_index, HashTable* ht, Record_f* rec, char* key)
{
...
unsigned char update_rec(int fd_data, HashTable* ht, Schema* sch, char* file_path,
			 Record_f* rec, char* buffer, unsigned char check, unsigned char update,
...
unsigned char __write(char* file_name, int* fd_index, int fd_data, char* data_to_add,
			Record_f* rec, char* key, indexing ixgn, int lock)
{
...
{	
	...
	free_strs(2,1,files);
	files = NULL;
	Schema sch = {0,NULL,NULL}, *psch = &sch;
